...
import sns.blog.javalin.controller.SocialMediaController;
...
public class UpdateMessageTextTest {
  ...
  /**
   * Before every test, reset the database, restart the Javalin app, and create a new webClient and
   * ObjectMapper for interacting locally on the web.
   *
  ...
  /**
   * Sending an http request to PATCH localhost:8080/messages/1 (message id exists in db) with
   * successfule message text
   *
   * <p>Expected Response: Status Code: 200 Response Body: JSON representation of the message that
   * was updated
   */
  @Test
  public void updateMessageSuccessful() throws IOException, InterruptedException {
    ...
    HttpResponse response =
        webClient.send(postMessageRequest, HttpResponse.BodyHandlers.ofString());
   ...
   *
   * <p>Expected Response: Status Code: 400 Response Body:
   */
  @Test
  ...
  /**
   * Sending an http request to PATCH localhost:8080/messages/1 (message text to update is an empty
   * string)
   *
   * <p>Expected Response: Status Code: 400 Response Body:
   */
